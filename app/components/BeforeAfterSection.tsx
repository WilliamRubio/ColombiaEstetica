    ...
    <section id="results" className="py-24 bg-white">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {placeholders.map((item, index) => (
            <div key={index} className="rounded-2xl overflow-hidden group relative aspect-[3/4]">
              ...
              >
                ...
                <div>
                  <p className="text-[#1a4a6b] font-bold text-sm">{item.procedure}</p>
                  <p className="text-gray-400 text-xs mt-0.5">Photo placeholder</p>
                </div>

                {/* Before/After labels */}
                <div className="absolute bottom-3 left-3 right-3 flex gap-2">
                  <span className="flex-1 text-center py-1 rounded-full text-xs font-bold bg-white/80 text-gray-600">Before</span>
                  <span className="flex-1 text-center py-1 rounded-full text-xs font-bold text-white" style={{ background: "#1a4a6b" }}>After</span>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="mt-8 flex items-start gap-3 p-4 rounded-xl" style={{ background: "#f9f6f0" }}>
          ...
          <p className="text-xs text-gray-500 leading-relaxed">
            <strong className="text-gray-700">Images shown with patient consent.</strong> All photographs are shared with full written authorization from our patients. Results vary per individual. These images are for illustrative purposes only and do not represent guaranteed outcomes.
          </p>
        </div>
      </div>
    </section>
